function Sync-ConfluenceSharePointInventory {
    ...
    .DESCRIPTION
        Creates or updates an M365 SharePoint & OneDrive page as a subpage under the
        specified parent hierarchy (defaults to 'Infrastructure Documentation > Cloud Services')
        in the Confluence space. The page displays storage summary with per-site details
        including URL, type, storage usage, and last modified date.

        The function:
        - Validates the target space exists
        - Finds or creates the parent page hierarchy
        - Searches for an existing M365 SharePoint & OneDrive page under that parent
    ...
    .PARAMETER PageTitle
        Title for the page. Defaults to 'M365 SharePoint & OneDrive'.
    ...
    .EXAMPLE
        ...
        Creates or updates the M365 SharePoint & OneDrive page under 'Infrastructure Documentation > Cloud Services' in the CONTOSO space.
    .EXAMPLE
        Sync-ConfluenceSharePointInventory -SpaceKey 'CONTOSO' -SharePointData $sites -WhatIf

        Shows what would be synced without making changes.
    .EXAMPLE
        Sync-ConfluenceSharePointInventory -SpaceKey 'CONTOSO' -SharePointData $sites -ParentPageTitle 'Storage'

        Creates M365 SharePoint & OneDrive under a custom parent page.
    .NOTES
    ...
    param(
        ...
        [Parameter()]
        [string]$PageTitle = 'M365 SharePoint & OneDrive',
